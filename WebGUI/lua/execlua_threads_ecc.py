...
LUA_EXEC_TIMES = 10
PROCESS_COUNT = 11
SCRIPT_FILE = "200206_32kbps_emulator_ecc.lua"  # your lua script filename
...
def jobs():  # register jobs to finish
    ...
    for ecc_bytes in range(8, 24, 8):
        for evaluate_idx in range(LUA_EXEC_TIMES):
            job = Job()
            job.output_filename = "job_%d_%d.txt" % (ecc_bytes, evaluate_idx)

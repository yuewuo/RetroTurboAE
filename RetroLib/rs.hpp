...
class ReedSolomon {
public:
    const uint8_t msg_length, ecc_length;
    ReedSolomon(const uint8_t msg_length_, const uint8_t ecc_length_):msg_length(msg_length_), ecc_length(ecc_length_)
    {
        const uint8_t   enc_len  = msg_length + ecc_length;
        const uint8_t   poly_len = ecc_length * 2;
        uint8_t** memptr   = &memory;
        uint16_t  offset   = 0;
        // printf("DEBUG: msg_len = %d, ecc_len = %d\n", msg_length, ecc_length);
     ...
     * @param *dst - output buffer for ecc     (ecc_length size at least) */
     void EncodeBlock(const void* src, void* dst) {
        ...
        static uint8_t generator_cache[127] = {0};
        ...
        if(generator_cached) {
            ...
            gen->Set(generator_cache, sizeof(uint8_t) * (ecc_length + 1));
        } else {
            GeneratorPoly();

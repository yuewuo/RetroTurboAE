...
def generate_prerequisites():
    ...
    global edge_prefix
    global data_rate_exp
    assert 0 == os.system(".\\Tester\\ExploreLCD\\EL_200206_GenerateSinusoidReference.exe %f %f" % (charging_edge, discharging_edge)), "run failed"
    ref_filename = "sinusoid_%dus_%dus.reordered" % (int(1e3*charging_edge), int(1e3*discharging_edge))

...
int main(int argc, char** argv) {
    ...
    if (argc != 5) {
		printf("usage: <local_filename> <collection> <id(12 byte hex = 24 char)> <sequence_key>\n");
        printf("for example: ./Tester/ExploreLCD/EL_200121_Emulator.exe 200115_model_500us_17mseq_9v.bin.resampled.reordered tmp <id> sequence\n");
		return -1;
	}
    ...
    const char* sequence_key = argv[4];

	MongoDat::LibInit();
	mongodat.open(MONGO_URL, MONGO_DATABASE);

    BsonOp record = mongodat.get_bsonop(collection_str, record_id_str);
	assert(record[sequence_key].existed() && record[sequence_key].type() == BSON_TYPE_UTF8);
    string sequence = record[sequence_key].value<string>();
    ...
    vector<bool> biseq;
    for (const char* it = sequence.c_str(); *it; ++it) {

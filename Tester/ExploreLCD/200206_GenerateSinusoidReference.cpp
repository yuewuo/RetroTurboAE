...
int main(int argc, char** argv) {
    ...
    float charging_rate = M_PI / charging_edge_t0 / 40;
    float discharging_rate = M_PI / discharging_edge_t0 / 40;
    float PI_2 = M_PI / 2;
    for (int j=0; j<30; ++j) {  // 30 repeat to simulate more accurately
        for (int i=20; i<vec_size; ++i) {
            int last_idx = ((vec_size-20) + i - 1) % (vec_size-20) + 20;
            float last_real = vec[last_idx].real();
            float last_sinusoid = last_real * 2 - 1;
            if (last_sinusoid < -1) last_sinusoid = -1;  // bound
            if (last_sinusoid > 1) last_sinusoid = 1;  // bound
            float last_phase = asin(last_sinusoid);
            bool now_signal = mseq17[(i/20-1) % mseq17.size()];
            float this_phase;
            if (now_signal) {  // driving to 1 using charging_edge_t0
                this_phase = last_phase + charging_rate;
                if (this_phase > PI_2) this_phase = PI_2;
            } else {  // driving to 1 using discharging_edge_t0
                this_phase = last_phase - discharging_rate;
                if (this_phase < -PI_2) this_phase = -PI_2;
            }
            float this_signal = sin(this_phase);
            float this_real = (this_signal + 1) / 2;
            if (this_real > 1) this_real = 1;
            if (this_real < 0) this_real = 0;
    ...
    char local_filename[256]; sprintf(local_filename, "sinusoid_%dus_%dus.reordered", (int)(1e3*charging_edge_t0), (int)(1e3*discharging_edge_t0));

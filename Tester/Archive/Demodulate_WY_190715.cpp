...
int main(int argc, char** argv) {
	...
	if (argc != 5) {
		printf("usage: <all_refs_id> <effect_length:int> <collection> <id(12 byte hex = 24 char)>");
		return -1;
	}

	const char* all_refs_id_str = argv[1];
	int effect_length = atoi(argv[2]);
...
#define EXTRA_TIME_S 4e-3
	vector<float> union_curve = union_curve_parse(binary, sample_rate, frequency, (samples.size() / frequency + EXTRA_TIME_S) * sample_rate);

...
/*
This file draws demod_buffer_length + 1 lines to a new graph, as a demonstration to our demodulation algorithm
*/
#define EMULATOR_KEEP_SEGMENT_WHEN_COPY
...
vector< vector< complex<float> > > draw_branches;
vector< vector< float > > draw_branches_var;
...
int main(int argc, char** argv) {
    ...
    draw_branches.resize(demod_buffer_length + 1);
    draw_branches_var.resize(demod_buffer_length + 1);
...
// 		upc(refs);
    ...
    // for (int i=0; i<1; ++i) {  // for debug
        ...
        // insert draw branches
        {  // add original signal here
            draw_branches[0].insert(draw_branches[0].end(), demod_data + start, demod_data + start + FastDSM_Emulator::seg_out_length);
            draw_branches_var[0].push_back(0);
            auto bt = branches.begin();
            for (int j=0; j < demod_buffer_length; ++j) {
                if (j < (int)branches.size()) {
                    draw_branches[j+1].insert(draw_branches[j+1].end(), bt->emu.out_segment.begin(), bt->emu.out_segment.end());
                    draw_branches_var[j+1].push_back(bt->segma);
                    // printf("bt->segma: %f\n", bt->segma);
                    ++bt;
                } else {
                    draw_branches[j+1].insert(draw_branches[j+1].end(), demod_data + start, demod_data + start + FastDSM_Emulator::seg_out_length);
                    draw_branches_var[j+1].push_back(0);
                }
            }
        }
    ...
    // upload draw graphs
    int single_draw_length = draw_branches[0].size();
    printf("single_draw_length: %d\n", single_draw_length);
    for (int i=1; i<=demod_buffer_length; ++i) {
        // printf("(int)draw_branches[i].size(): %d\n", (int)draw_branches[i].size());
        assert(single_draw_length == (int)draw_branches[i].size());
    }
    vector< complex<float> > all_branches_draw; all_branches_draw.resize(single_draw_length * (1+demod_buffer_length));
    for (int i=0; i<single_draw_length; ++i) {
        for (int j=0; j<=demod_buffer_length; ++j) {
            all_branches_draw[i*(1+demod_buffer_length) + j] = draw_branches[j][i];
        }
    }
    string y_labels;
    for (int i=0; i<=demod_buffer_length; ++i) {
        if (i!=0) y_labels += ",";
        char buf[32]; sprintf(buf, "I%d,Q%d", i, i);
        y_labels += buf;
    }
    mongodat.upload_record("all_branches", (float*)all_branches_draw.data(), 2 * (1+demod_buffer_length), all_branches_draw.size() / (1+demod_buffer_length), NULL, "all_branches", 1/80., "time(ms)", 1, y_labels.c_str());
    printf("upload all_branches file with ID: %s\n", MongoDat::OID2str(mongodat.get_fileID()).c_str());

    // upload variance data
    int single_var_length = draw_branches_var[0].size();
    printf("single_var_length: %d\n", single_var_length);
    for (int i=1; i<=demod_buffer_length; ++i) {
        // printf("(int)draw_branches_var[i].size(): %d\n", (int)draw_branches_var[i].size());
        assert(single_var_length == (int)draw_branches_var[i].size());
    }
    vector< float > all_branches_var; all_branches_var.resize(single_var_length * (1+demod_buffer_length));
    for (int i=0; i<single_var_length; ++i) {
        for (int j=0; j<=demod_buffer_length; ++j) {
            all_branches_var[i*(1+demod_buffer_length) + j] = draw_branches_var[j][i];
        }
    }
    string y_labels_2;
    for (int i=0; i<=demod_buffer_length; ++i) {
        if (i!=0) y_labels_2 += ",";
        char buf[32]; sprintf(buf, "v%d", i);
        y_labels_2 += buf;
    }
    mongodat.upload_record("all_branches_var", (float*)all_branches_var.data(), (1+demod_buffer_length), all_branches_var.size() / (1+demod_buffer_length), NULL, "all_branches_var", 4, "time(ms)", 1, y_labels_2.c_str());
    printf("upload all_branches_var file with ID: %s\n", MongoDat::OID2str(mongodat.get_fileID()).c_str());

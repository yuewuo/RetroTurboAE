...
// printf("upload out_segment_0 file with ID: %s\n", MongoDat::OID2str(mongodat.get_fileID()).c_str());
        ...
        if (with_dc) {
            ...
            // this point should be singles[i]
            double all = bit_per_symbol == 6 ? 14 : 15;
            double I_should_be = 0; for (int j=0; j<4; ++j) { if (singles[i] & (0x01<<j)) I_should_be += (8>>j); } I_should_be /= all;
            double Q_should_be = 0; for (int j=0; j<4; ++j) { if (singles[i] & (0x10<<j)) Q_should_be += (8>>j); } Q_should_be /= all;
            scatter_sum += (I_should_be - I)*(I_should_be - I) + (Q_should_be - Q)*(Q_should_be - Q);
            scatter_count++;
            // scatter_output.push_back(complex<float>(I_should_be-I, Q_should_be-Q));
        } else {
    ...
    int wpr_output_length = emulated.size() - FastDSM_Emulator::bias;
    vector<float> wpr_output; wpr_output.resize(4 * wpr_output_length);
    for (int i=0; i<wpr_output_length; ++i) {
        int a = 4*i;
        wpr_output[a] = emulated[FastDSM_Emulator::bias + i].real();
        wpr_output[a+1] = emulated[FastDSM_Emulator::bias + i].imag();
        wpr_output[a+2] = scatter_data[FastDSM_Emulator::bias + i].real();
        wpr_output[a+3] = scatter_data[FastDSM_Emulator::bias + i].imag();
        diff_sum = (wpr_output[a]-wpr_output[a+2])*(wpr_output[a]-wpr_output[a+2]) + (wpr_output[a+1]-wpr_output[a+3])*(wpr_output[a+1]-wpr_output[a+3]);
    }
    // mongodat.upload_record("wpr_output", (float*)wpr_output.data(), 4, wpr_output_length, NULL, "wpr_output", 1/80., "time(ms)", 1, "Ie,Qe,Ir,Qr");
    // printf("upload wpr_output file with ID: %s\n", MongoDat::OID2str(mongodat.get_fileID()).c_str());

    // record["wpr_output_id"] = MongoDat::OID2str(mongodat.get_fileID());
    record["diff_avr"] = sqrt(diff_sum / wpr_output_length);
    record["scatter_stddev"] = sqrt(scatter_sum / scatter_count);
    record.save();
